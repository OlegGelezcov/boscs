    ...
    , IComparable<BigDecimal>
{
    ...
    public BigInt Mantissa { get; set; }
    public int Exponent { get; set; }

    public BigDecimal(BigInt mantissa, int exponent)
        : this()
    {
        Mantissa = mantissa;
        Exponent = exponent;
    ...
    public void Normalize()
    {
        ...
        else
        {
            ...
            {
                BigInt shortened = nu;
                BigInt.Divide(Mantissa, 10, out shortened, out remainder);
                if (remainder == 0)
                {
                    Mantissa = shortened;
                    Exponent++;
    ...
    public static int NumberOfDigits(BigInt value)
    {
        // do not count the sign
        //return (value * value.Sign).ToString().Length;
        // faster version
        return (int)Math.Ceiling(BigInt.Log10(value * value.Sign));
    }
    ...
    public static implicit operator BigDecimal(double value)
    {
        ...
        while (Math.Abs(value * scaleFactor - (double)mantissa) > 0)
        {
            exponent -= 1;
            scaleFactor *= 10;
            mantissa = (BigInt)(value * scaleFactor);
        }
    ...
    {
        return (int)(value.Mantissa * BigInt.Pow(10, value.Exponent));
    }

    public static explicit operator uint(BigDecimal value)
    {
        return (uint)(value.Mantissa * BigInt.Pow(10, value.Exponent));
    }
    ...
    public static BigDecimal operator /(BigDecimal dividend, BigDecimal divisor)
    {
        ...
        dividend.Mantissa *= BigInt.Pow(10, exponentChange);
    ...
    /// </summary>
    private static BigInt AlignExponent(BigDecimal value, BigDecimal reference)
    {
        return value.Mantissa * BigInt.Pow(10, value.Exponent - reference.Exponent);
    }
