...
public class SpawnManager : MonoBehaviour {
	...
	void Start () 
	{
		...
		playerTransform = FindObjectOfType<PlayerMotor>().transform; //GameObject.FindGameObjectWithTag ("Player").transform;
		_spawnCount = 0;
		_nextCoin = Random.Range(20, 60);
	...
	public void ReplaceTile(GameObject source, GameObject target) {
		int index = -1;
		index = activeTiles.FindIndex((go) => go == source);
		if (index >= 0) {
			activeTiles.Insert(index, target);
		}
		Destroy(source);
	}
	...
	int RandomPrefabIndex()
	{
		...
		while ((randomIndex == lastPrefabIndex) 
			|| (randomIndex == 2 && lastPrefabIndex == 0) 
		    || (randomIndex == 2 && lastPrefabIndex == 4) 
			|| (randomIndex == 4 && lastPrefabIndex == 0) 
		    || (randomIndex == 4 && lastPrefabIndex == 2) 
			|| (randomIndex == 1 && lastPrefabIndex == 3) 
		    || (randomIndex == 1 && lastPrefabIndex == 5)
			|| (randomIndex == 3 && lastPrefabIndex == 1) 
		    || (randomIndex == 3 && lastPrefabIndex == 5)
			|| (randomIndex == 5 && lastPrefabIndex == 1) 
		    || (randomIndex == 5 && lastPrefabIndex == 3)
			|| (randomIndex == 6 && lastPrefabIndex == 7) 
		    || (randomIndex == 6 && lastPrefabIndex == 8)
			|| (randomIndex == 7 && lastPrefabIndex == 6) 
		    || (randomIndex == 7 && lastPrefabIndex == 8)
			|| (randomIndex == 8 && lastPrefabIndex == 6) 
		    || (randomIndex == 8 && lastPrefabIndex == 7)
		)
		{
			if (_spawnCount > _nextCoin && lastPrefabIndex != 2 && lastPrefabIndex != 4)
			{
				randomIndex = CoinTile();
			}
			else
			{
	...
	private int CoinTile()
	{
		_nextCoin = _nextCoin+=Random.Range(20, 60);
		return 0;
	}
}
}