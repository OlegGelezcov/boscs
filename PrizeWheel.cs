	...
	public int LoosIndex = 15;
		...
		public override void ApplyLayout()
		{
			...
			if (_Selector) 
			{
				...
				mSelectorElasticStartAngle = _Selector.localRotation.eulerAngles.z;
			}
		...
		public override void StartRound()
		{
			...
			if (Random.Range(0, 1f) > WinChance)
			{
				mForceReward = LoosIndex;
			}
		...
		private void GetRandomElement( ref int ElementIdx, out float Angle )
		{
			...
			bool first = true;
			float TotalVisualPriority = mRewards.Count;
		...
		private IEnumerator DoPlay( int TargetElementIdx, float ElementAngle )
		{
			...
			{
				InitialAngle = -_Selector.localRotation.eulerAngles.z;
			}
			else
			{
				InitialAngle = _Wheel.localRotation.eulerAngles.z;
			}
			...
			while(true)
			{
				...
				if (finished && !rotateback && _Selector && mKnots.Count>0) 
				{
					...
					float selectorDAngle = Vector3.Angle(_Selector.localPosition, _Selector.localRotation * Vector3.down);
		...
		private bool UpdateSelector( ref int currentKnot, bool rotateback )
		{
			...
			{
				var knot = mKnots[i];
				vKnotPos = _Wheel.localRotation * knot.localPosition;
			...
			float currentSelectorAngle = _Selector.localRotation.eulerAngles.z;
			bool finished = false;
			...
			_Selector.localRotation = Quaternion.Euler(0,0, currentSelectorAngle);
			return finished;
		}
